...
class ClaimStore:
    ...
    def create(self, claim: KnowledgeClaim) -> KnowledgeClaim:
        ...
        with self._session_factory() as session:
            ...
            try:
                ...
                session.rollback()
                if _is_duplicate_evidence_error(exc):
                    raise DuplicateEvidenceError(
                        f"Evidence id attached to claim '{claim.claim_id}' already exists."
                    ) from exc
                raise DuplicateClaimError(f"Claim '{claim.claim_id}' already exists.") from exc
        return claim
    ...
    def list_evidence(self, *, claim_id: str | None = None) -> list[Evidence]:
        statement = select(EvidenceRecord)
        if claim_id is not None:
            statement = statement.where(EvidenceRecord.claim_id == claim_id)
        statement = statement.order_by(EvidenceRecord.evidence_id)
    ...
    def list_canonical_tool_specs(self) -> list[ToolSpec]:
        with self._session_factory() as session:
            records = session.scalars(
                select(CanonicalToolSpecRecord).order_by(CanonicalToolSpecRecord.tool_id)
            ).all()
    ...
    def list_canonical_workflow_specs(self) -> list[WorkflowSpec]:
        with self._session_factory() as session:
            records = session.scalars(
                select(CanonicalWorkflowSpecRecord).order_by(CanonicalWorkflowSpecRecord.workflow_id)
            ).all()
...
def _is_duplicate_evidence_error(exc: IntegrityError) -> bool:
    message = str(exc.orig).lower()
    return "evidence.evidence_id" in message or "evidence_id" in message
