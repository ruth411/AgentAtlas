...
Auth is opt-in via `AGENTATLAS_API_KEY`. When unset (the dev default),
...
Separately, the reviewer registry (`AGENTATLAS_REVIEWER_REGISTRY`) gates
...
def client(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("AGENTATLAS_REVIEWER_REGISTRY", raising=False)
...
def test_reads_remain_public_when_auth_enabled(client) -> None:
    """The point of AgentAtlas is to be queryable. Read endpoints stay
...
def test_unregistered_reviewer_rejected(client) -> None:
    test_client, monkeypatch = client
    monkeypatch.setenv("AGENTATLAS_REVIEWER_REGISTRY", "alice,bob")
